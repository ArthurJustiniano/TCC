...
create table if not exists public.inbox (
  ...
  text text not null,
  recipient_id int not null references public.usuario(id_usuario) on delete cascade,
  created_at timestamptz not null default now()
);

create index if not exists idx_inbox_recipient_id on public.inbox (recipient_id);

-- 2) Realtime: adicionar à publicação
-- (ignora erro se já estiver adicionada)
do $$
begin
  begin
  ...
  begin
    execute 'alter publication supabase_realtime add table public.inbox';
  exception when others then null;
  end;
end$$;

-- 3) RLS e políticas (Realtime respeita RLS)
alter table public.news enable row level security;
alter table public.inbox enable row level security;

-- Políticas de leitura (permitem SELECT para anon/auth; o filtro por usuário será feito no subscribe com recipient_id)
do $$
begin
  if not exists (
    select 1 from pg_policies where schemaname = 'public' and tablename = 'news' and policyname = 'news_select_all'
  ) then
  ...
  if not exists (
    select 1 from pg_policies where schemaname = 'public' and tablename = 'inbox' and policyname = 'inbox_select_all'
  ) then
    create policy "inbox_select_all" on public.inbox
      for select to anon, authenticated
      using (true);
  end if;
end$$;

-- 4) Trigger: espelhar mensagens do chat em inbox para notificar receptor
create or replace function public.fn_messages_to_inbox()
returns trigger
language plpgsql
as $$
begin
  insert into public.inbox (from_name, text, recipient_id, created_at)
  values (
    coalesce(new.sender_username, 'Usuário ' || new.sender_id::text),
    new.message,
    new.receiver_id,
...
begin
  if exists (
    ...
    where tgname = 'tr_messages_to_inbox' and tgrelid = 'public.messages'::regclass
  ) then
    execute 'drop trigger tr_messages_to_inbox on public.messages';
  end if;
  execute '
    create trigger tr_messages_to_inbox
    after insert on public.messages
    for each row execute function public.fn_messages_to_inbox()
  ';
end$$;

-- 5) (Opcional) Dados de teste
insert into public.news (titulo, resumo) values ('Teste News', 'Resumo de teste');
-- Substitua 1 pelo id de um usuário válido em public.usuario
insert into public.inbox (from_name, text, recipient_id) values ('Sistema', 'Mensagem privada de teste', 1);
